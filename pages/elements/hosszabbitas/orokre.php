...
$userRankPriceBe = $detailsRank["be_ertek_orokre"];
$userRankPriceForint = $detailsRank["forint_ertek_orokre"];
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if(isset($_POST["forint-buy"])) {
        if($userRankPriceForint === 0) return header("Location: orokre.php?errorCode=Ezt nem aktiválhatod forintból.");
        if($userRankPriceForint > $forint) return header("Location: orokre.php?errorCode=Neked nincs elég forintod erre.");
        if($rangExpire == "Örök") return header("Location: orokre.php?errorCode=A te rangodat már örökre megvásároltad.");
        ...
        header("Location: orokre.php?successCode=Sikeresen meghosszabbítottad a rangodat még egy hónapra.");
        exit();
    }

    //bevel akarja megvenni #nerd
    if(isset($_POST["banyaszerme-buy"])) {
        if($userRankPriceBe === 0) return header("Location: orokre.php?errorCode=Ezt nem aktiválhatod bányászérméből.");
        if($userRankPriceBe > $banyaszerme) return header("Location: orokre.php?errorCode=Neked nincs elég bányászérméd erre.");
        if($rangExpire == "Örök") return header("Location: orokre.php?errorCode=A te rangodat már örökre megvásároltad.");
        ...
        mysqli_stmt_execute($updateRankSqlPrepare);
        header("Location: orokre.php?successCode=Sikeresen megvetted a rangodat örökre.");
        exit();
    }
}
...
<body>
    <div class="container">
        <div class="elfelejtett-jelszo">
            <h2><span>MesterMC Örök</span> Rang hosszabbítás</h2>
