...
if($_SERVER["REQUEST_METHOD"] == "POST") 
{
    if(isset($_FILES["kep-feltoltes"]) && !isset($_POST["delete"])) 
    {
        ...
        if($_FILES['kep-feltoltes']['error'] != UPLOAD_ERR_OK) return header("Location: szarny.php?errorCode=Hiba történt.");

        $fileType = exif_imagetype($uploadedPic['tmp_name']);

        if($fileType !== IMAGETYPE_PNG) return header("Location: szarny.php?errorCode=Csak PNG típusú képeket fogadunk el.");
        list($width, $height) = getimagesize($uploadedPic['tmp_name']);
        if($width != 64 && $height != 32) return header("Location: szarny.php?errorCode=Csak 64x32-s szárnyat lehet feltölteni.");
    
        $fileName = $username . "_szarny.png"; 
        $dir = "../../../assets/media/kellekek/szarnyak/";
        $target = $dir . $fileName;

        if(move_uploaded_file($uploadedPic['tmp_name'], $target)) 
        {
            $uploadSkinQuery = "UPDATE kellekek SET szarny = ? WHERE username = ?";
            ...
            header("Location: szarny.php?successCode=Sikeres szárny feltöltés.");
            exit();
        }
        else
        {
            header("Location: szarny.php?errorCode=Hiba történt a szárny feltöltése során.");
            exit();
        }
    }
    ...
    if(isset($_POST["delete"]))
    {
        ...
        if(!file_exists($target)) return header("Location: szarny.php?errorCode=Neked nincs feltöltve szárny.");

        try {
            unlink($target);
        } catch (\Throwable $th) {
            header("Location: szarny.php?errorCode=$th.");
            exit();
        }

        $uploadSkinQuery = "UPDATE kellekek SET szarny = NULL WHERE username = ?";
        $uploadSkinQueryPrepare = mysqli_prepare($connection, $uploadSkinQuery);
        mysqli_stmt_bind_param($uploadSkinQueryPrepare, "s", $username);
        mysqli_stmt_execute($uploadSkinQueryPrepare);
        header("Location: szarny.php?successCode=Sikeres szárny törlés.");
        exit();
    }
}
