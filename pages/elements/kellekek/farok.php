...
if($_SERVER["REQUEST_METHOD"] == "POST") 
{
    if(isset($_FILES["kep-feltoltes"]) && !isset($_POST["delete"])) 
    {
        ...
        if($_FILES['kep-feltoltes']['error'] != UPLOAD_ERR_OK) return header("Location: farok.php?errorCode=Hiba történt.");

        $fileType = exif_imagetype($uploadedPic['tmp_name']);

        if($fileType !== IMAGETYPE_PNG) return header("Location: farok.php?errorCode=Csak PNG típusú képeket fogadunk el.");
        list($width, $height) = getimagesize($uploadedPic['tmp_name']);
        if($width != 64 && $height != 32) return header("Location: farok.php?errorCode=Csak 64x32-s farkat lehet feltölteni.");
    
        $fileName = $username . "_farok.png"; 
        $dir = "../../../assets/media/kellekek/farkak/";
        $target = $dir . $fileName;

        if(move_uploaded_file($uploadedPic['tmp_name'], $target)) 
        {
            $uploadSkinQuery = "UPDATE kellekek SET farok = ? WHERE username = ?";
            ...
            header("Location: farok.php?successCode=Sikeres farok feltöltés.");
            exit();
        }
        else
        {
            header("Location: farok.php?errorCode=Hiba történt a farok feltöltése során.");
            exit();
        }
    }
    ...
    if(isset($_POST["delete"]))
    {
        ...
        if(!file_exists($target)) return header("Location: farok.php?errorCode=Neked nincs feltöltve skin.");

        try {
            unlink($target);
        } catch (\Throwable $th) {
            header("Location: farok.php?errorCode=$th.");
            exit();
        }

        $uploadSkinQuery = "UPDATE kellekek SET farok = NULL WHERE username = ?";
        $uploadSkinQueryPrepare = mysqli_prepare($connection, $uploadSkinQuery);
        mysqli_stmt_bind_param($uploadSkinQueryPrepare, "s", $username);
        mysqli_stmt_execute($uploadSkinQueryPrepare);
        header("Location: farok.php?successCode=Sikeres farok törlés.");
        exit();
    }
}
