...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    {
        if($banyaszerme < $udvozloErtekBe) return header("Location: nevelo.php?errorCode=Neked nincs elég bányászérméd erre.");
    ...
    {
        if($forint < $udvozloErtekForint) return header("Location: nevelo.php?errorCode=Neked nincs elég forintod erre.");
    ...
    if(isset($_POST["submit-inputs"]))
    {
        ...
        if (empty($elso)) {
            return header("Location: nevelo.php?errorCode=Kérlek töltsd ki a mezőt.");
        }
        
        if (strlen($elso) > $max_length)  return header("Location: nevelo.php?errorCode=Legfeljebb 12 karakter lehet.");
        
        $updateUdvozloSql = "UPDATE extrak SET nevelo = ? WHERE username = ?";
        $preapreUpdate = mysqli_prepare($connection, $updateUdvozloSql);
        mysqli_stmt_bind_param($preapreUpdate, "ss", $elso, $username);
        mysqli_stmt_execute($preapreUpdate);

        header("Location: nevelo.php?successCode=Sikeresen megváltoztattad a névelőd.");
        exit();
    }
}
            ...
            <div class="activated">
                <form id="activated-form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                    ...
                    <?php elseif($successCode != ""): ?>
                        <p class="success"><?php echo $_GET["successCode"] ?></p>           
                    <?php endif; ?>
                        <input type="text" name="elso" id="" placeholder="Névelő" maxlength="12">
                    <div class="buttons">
