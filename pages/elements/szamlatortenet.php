...
$tablaQuery = "SELECT * FROM utalasok WHERE `utalas_kezdemenyzo` = ? OR `utalas_megkapo` = ?";
$tablaPrepare = mysqli_prepare($connection, $tablaQuery);
mysqli_stmt_bind_param($tablaPrepare, "ss", $username, $username);
mysqli_stmt_execute($tablaPrepare);
$results = mysqli_stmt_get_result($tablaPrepare);
...
<head>
    ...
    <link rel="stylesheet" href="../../assets/css/pages/elements/szamlatortenet.css">
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="korona-exchange">
            <table>
                <h2 class="szamlatortenet-h2"><span>MesterMC</span> Számlatörténet</h2>
                <tr>
                    <th>Címzett</th>
                    <th>Küldő</th>
                    <th>Típus</th>
                ...
                <?php
                    ...
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($row['utalas_megkapo']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['utalas_kezdemenyzo']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['tipus']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['datum']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['ertek']) . "</td>";
                        if($row['statusz'] == 'Elfogadva')
                            echo "<td class='elfogadva-statusz'>" . htmlspecialchars($row['statusz']) . "</td>";
                        else
                            echo "<td>" . htmlspecialchars($row['statusz']) . "</td>";
                    echo "</tr>";
                    }
                ?>
                </table>
